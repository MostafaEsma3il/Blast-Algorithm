...
for i in range(1, sz1 + 1):
    for j in range(1, sz2 + 1):
        ...
        algin_matrix[i][j] = max(protein,left, up, diagonal,0)
        if mx < algin_matrix[i][j]:
            mx = algin_matrix[i][j]
            maxi = i
            maxj = j
...
i=maxi
j=maxj
while(i>0 and j>0 and algin_matrix[i][j]!=0):
...
####the Upcoming code is the Global Aligment code as there's no a vacancy to submit global in form##########
